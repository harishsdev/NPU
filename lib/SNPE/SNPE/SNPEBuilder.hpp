...
/**
 * @brief Sets the async execution queue depth for QNN backends.
 *        A value of 0 (default) disables async execution.
 *        A non-zero value enables QnnGraph_executeAsync with the
 *        specified queue depth.
 *
 * @param[in] queueDepth Maximum number of async execute calls
 *            that can be enqueued before blocking.
 *
 * @return The current instance of SNPEBuilder.
 */
  SNPEBuilder& setAsyncExecuteQueueDepth(uint32_t queueDepth){
    Snpe_SNPEBuilder_SetAsyncExecuteQueueDepth(handle(), queueDepth);
    return *this;
  }
...
/**
 * @brief Sets the file read memory budget for the SNPE instance.
 *
 * @param[in] fileReadMemoryBudgetInMb The file read memory budget in MB.
 *
 * @return The current instance of SNPEBuilder
 */
  SNPEBuilder& setFileReadMemoryBudget(uint64_t fileReadMemoryBudgetInMb){
    Snpe_SNPEBuilder_SetFileReadMemoryBudget(handle(), fileReadMemoryBudgetInMb);
    return *this;
  }

/**
 * @brief Registers multi-context spill-fill buffer sharing with the HTP backend.
 *        Both groupId and maxSpillFillBuffer must be provided together.
 *        Only applies to the DSP runtime; ignored otherwise.
 *
 * @param[in] groupId Group handle (as uint64_t). Pass 0 to create a new group
 *            (this instance becomes the group leader). Pass the group id
 *            value of an existing group leader to join that group.
 *
 * @param[in] maxSpillFillBuffer Maximum spill-fill buffer size in bytes shared
 *            across all contexts in the group.
 *
 * @return The current instance of SNPEBuilder
 */
  SNPEBuilder& registerMultiContext(uint64_t groupId, uint64_t maxSpillFillBuffer){
    Snpe_SNPEBuilder_RegisterMultiContext(handle(), groupId, maxSpillFillBuffer);
    return *this;
  }
...
/**
 * @brief Sets deferred Init
 *
 * @param[in] deferredInit, true implies deferredInit is enabled
 *
 * @return SNPE_SUCCESS upon successfully applying deferred Init
 */
...
/**
 * @brief Sets deferred backend cleanup
 *
 * @param[in] deferredCleanup, true implies deferredCleanup is enabled
 *
 * @return The current instance of SNPEBuilder
 *
 * @note Deferred cleanup of backend resources. The default is false. When enabled,
 *       the backend resources (e.g., HTP/GPU/CPU) will not be released during de-init.
 *       Subsequent re-inits will also be faster. The user is responsible for cleaning
 *       up backend resources via calling SNPEFactory::cleanupRuntime()
 */
  SNPEBuilder& setDeferredRuntimeCleanup(bool deferredCleanup){
    Snpe_SNPEBuilder_SetDeferredRuntimeCleanup(handle(), deferredCleanup);
    return *this;
  }
