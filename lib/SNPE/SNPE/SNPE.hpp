...
/**
 * @brief Async variant of execute(networkName, UserBufferMap, UserBufferMap).
 ...
 *        queue depth to be set via setAsyncExecuteQueueDepth.
 *
 * @param[in] networkName Network name.
 * @param[in] input Input UserBufferMap.
 * @param[in] output Output UserBufferMap.
 ...
 *
 * @return true if execution was successfully enqueued.
 */
  bool executeAsync(const char* networkName,
                    const DlSystem::UserBufferMap& input,
                    const DlSystem::UserBufferMap& output,
                    void (*notifyFn)(void* notifyParam, bool success),
                    void* notifyParam) noexcept{
    return SNPE_SUCCESS == Snpe_SNPE_ExecuteUserBuffersForNetworkAsync(
        handle(), networkName, getHandle(input), getHandle(output), notifyFn, notifyParam);
  }
 ...
 */
  ...
  */
   ...
   *        The returned value can be passed directly as the groupId parameter to
   *        SNPEBuilder::registerMultiContext() so a second SNPE instance can join this
   *        instance's spill-fill sharing group.
   *
   * @param[in]  networkName   Name of the network graph
   * @param[in]  runtime       Runtime whose context handle to retrieve
   * @param[out] contextHandle Receives the QNN context handle as uint64_t
   *
   * @return true on success; false if the network is not found or has no matching subnet.
   */
  bool getContextHandle(const std::string& networkName, zdl::DlSystem::Runtime_t runtime, uint64_t& contextHandle) {
    return SNPE_SUCCESS == Snpe_SNPE_GetContextHandleForNetwork(handle(), networkName.c_str(),
                                                                static_cast<Snpe_Runtime_t>(runtime), &contextHandle);
  }
 ...
 *
 * @param[in] networkName  Name of the network; pass "" to use the first network.
 * @param[in] enable       If true, enables VTCM retention.
 *
 * @return true if at least one HTP subnet was found and updated; false otherwise.
 */
  bool setDspVtcmRetention(const char* networkName, bool enable) noexcept {
    return SNPE_SUCCESS == Snpe_SNPE_SetDspVtcmRetention(handle(), networkName, enable);
  }
